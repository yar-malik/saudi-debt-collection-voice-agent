...
const said = process.argv.slice(2).join(' ') || 'أبي أعرف كم المتبقي علي ومتى آخر موعد'
...
if (!agentId) {
  ...
  const audio = await voho('/v1/speech', { text: 'أهلاً، معك ليلى من النخبة بخصوص فاتورة مستحقة. هل هذا وقت مناسب؟', voice: 'layla', model: 'sada-1', format: 'mp3' }, true)
...
const out = await voho(`/v1/agents/${agentId}/reply`, {
  text: said,
  variables: { company: process.env.VOHO_COMPANY ?? 'النخبة' },
})
